...
<?php
	...
	function kisaMetin($metin, $uzunluk){
		strip_tags($metin);
	...
	function tarih($tarihim){
		setlocale(LC_TIME,'turkish');
		return iconv('latin5','utf-8',strftime('%d %B %Y', strtotime($tarihim)));
	}

	function saat($saat){
		setlocale(LC_TIME,'turkish');
		return iconv('latin5','utf-8',strftime('%H:%M', strtotime($saat)));
	}
	
	function hit($hitim){
		return number_format($hitim, 0, ',', '.');
	}
	
	function sifre($sifre){
		return md5(sha1(md5($sifre)));
	}
