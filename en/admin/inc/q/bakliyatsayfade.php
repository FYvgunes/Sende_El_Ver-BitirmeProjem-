...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Bakliyat DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=bakliyatsayfa-eklede"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  bakliyatsaydade SET salcalar_text='{$sayfa_text}',bakliyat_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=bakliyatsayfade");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  bakliyatsaydade SET  bakliyat_url	='{$hedef1}',bakliyat_title='{$sayfa_title}',bakliyat_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=bakliyatsayfade");
        } else {
...
$setting = $db->query("SELECT * FROM bakliyatsaydade ")->fetch(PDO::FETCH_ASSOC);
?>
