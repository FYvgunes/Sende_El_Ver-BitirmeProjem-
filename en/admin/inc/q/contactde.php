...
if ($_POST) {
    ...
    if (empty($site_adres)) {
        ...
        $update = $db->prepare("UPDATE contactde SET contact_adres='{$site_adres}', contact_tel='{$site_tel}',contact_fax='{$site_fax}',  contact_email='{$site_email}'");
        $update->execute();
        echo "<p class='alert alert-success'>Ayarlar başarıyla düzenlendi.</p>";
    }
}

$setting = $db->query("SELECT * FROM contactde")->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-edit"></i> İletişim Kısmını Düzenle Düzenle DE</h2>
        <div class="item row">
