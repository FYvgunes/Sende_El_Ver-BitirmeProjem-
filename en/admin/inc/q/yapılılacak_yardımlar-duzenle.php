...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  yapılacak_yardımlar   SET yapılılacak_yardımlar_p=0, yapılacak_yardımlar_baslik='{$Konala_baslik}', yapılılacak_yardımlar_text ='{$Konala_text}', yapılılacak_yardımlar_adres='{$Konala_title}', yardimci_id='{$yardim_id}' , yapılacak_yardımlar_tarih ='{$Konala_date}' WHERE yapılılacak_yardımlar_id={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  yapılacak_yardımlar   SET  yapılılacak_yardımlar_p =0, yapılacak_yardımlar_baslik='{$Konala_baslik}',   yapılılacak_yardımlar_url ='{$hedef1}', yapılılacak_yardımlar_adres='{$Konala_title}', yapılılacak_yardımlar_text ='{$Konala_text}'  WHERE yapılılacak_yardımlar_id='{$id}'");
