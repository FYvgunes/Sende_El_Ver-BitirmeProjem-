...
<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-file-text-o"></i> Bakliyat EN<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=bakliyat-ekleen"; ?>"><i class="fa fa-plus"></i> İçerik Ekle</a></small><br> <small><a href="<?php echo URL . "/en/admin/index.php?do=bakliyatsayfaen"; ?>"><i class="fa fa-plus"></i> Sayfa Yapısını Değiştir</a></small></h2>
        <div class="item row">
            <?php
            $sorgu = $db->query("SELECT bakliyat_id FROM bakliyaten WHERE bakliyat_p=0 ", PDO::FETCH_ASSOC);
            ...
            $row = $db->query("SELECT * FROM bakliyaten WHERE bakliyat_p=0  ORDER BY bakliyat_id DESC  LIMIT $baslangic, $limit", PDO::FETCH_ASSOC);
            foreach ($row as $content) {
            ?>
                <div class="list-group-item list-trend">
                    <div class="clearfix content-heading">
                        <img class="pull-left img-trend" src="<?php echo $content['bakliyat_url']; ?>" />
                        <h3 class="txt-trend">Bakliyat <?php echo $content['bakliyat_id']; ?> <br /><small><a target="_blank" href="<?php echo URL . "/en/" . $content['bakliyat_url']; ?>"><i class="fa fa-eye"></i> Sayfayı Göster</a> | <a href="<?php echo URL . "/en/admin/index.php?do=bakliyat-duzenleen&id=" . $content['bakliyat_id']; ?>"><i class="fa fa-edit"></i> Düzenle</a> | <a onclick="return confirm('İçeriği silmek istediğinizden emin misiniz?');" href="<?php echo URL . "/en/admin/index.php?do=bakliyat-silen&id=" . $content['bakliyat_id']; ?>"><i class="fa fa-trash-o"></i> Sil</a></small></h3>
                    </div>
                </div>
            <?php
            }
